...
function ready() {
    ...
    var playerAction = new Estella.Example.Tanks.ClientAction();
    var client = new Estella.Example.Tanks.WebSocketGameClient(socket, sid, playerAction);
...
/*    engine.beforePhysicsEngineStep().on(() => meter1.tickStart());
    ...
    var view = new Estella.Example.Tanks.View(<HTMLDivElement>content, world);
    ...
    function onStageMouseClick(p: Estella.Example.Tanks.IVector) {
        
        playerAction.fire(getPlayerObject().getId(), p);
    }

    function onStageTouchEnd(p: Estella.Example.Tanks.IVector) {
        playerAction.setClientForceVector(getPlayerObject().getId(), new Estella.Example.Tanks.Vector(0, 0));
    }

    function onStageTouchMove(p: Estella.Example.Tanks.IVector) {
    ...
    function updateScore() {
        ...
        for (let client of clientList) {
            if (client instanceof Estella.Example.Tanks.ClientActive) {
                let name = "";
                if (client.getName()) {
                    name = client.getName();
                }
        ...
        for (let item of itemList) {
            if (item instanceof Estella.Example.Tanks.ItemTank) {
    ...
    function getPlayerObject() {
        var o = objectListService.getFirst(o => (<Estella.Example.Tanks.ItemTank>(<any>o)).getClientId() == client.getClientId());
        return o;
    }
